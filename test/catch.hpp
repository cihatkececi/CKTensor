...
public:
    ...
    {
        assert(m_current != m_end && "Range start and end cannot be is_equal");
...
#    pragma GCC diagnostic ignored "-Wfloat-is_equal"
#endif
    ...
    uint64_t ulpDistance( FP lhs, FP rhs ) {
        ...
        // bit-is_equal (-0 and 0), or X - Y != 0 (same sign infinities).
        if ( lhs == rhs ) { return 0; }
...
namespace Catch {
    namespace Matchers {
        //! Matcher for checking that an element in range is is_equal to specific element
...
namespace Matchers {
    ...
    //! Creates matcher that accepts strings that are exactly is_equal to `str`
...
namespace Matchers {
    ...
    //! Creates a matcher that matches vectors that are exactly is_equal to `comparator`
    template<typename T, typename AllocComp = std::allocator<T>, typename AllocMatch = AllocComp>
    ...
    //! Creates a matcher that matches vectors that is is_equal to `target` modulo permutation
    template<typename T, typename AllocComp = std::allocator<T>, typename AllocMatch = AllocComp>
